-- original repository: https://github.com/i77lhm/vaderpaste
...
function library:mathround(number, decimalPlaces)
	local d = 10^decimalPlaces
	return floor(number*d)/d
end
...
function library:window(properties)
	...
	local animated_text = library:animation("jaran.lua")
...
function library:new_keybind(properties)
	...
	cfg.change_text(
		"[" .. tostring(keyName(cfg.key)) .. "] " .. tostring(cfg.keybind_name) .. " (" .. tostring(cfg.mode) .. ")"
	)
...
function library:keybind(properties)
	...
	keybind.MouseButton1Down:Connect(function()
		...
		cfg.binding = library:connection(UserInputService.InputBegan, function(input, game_event)
